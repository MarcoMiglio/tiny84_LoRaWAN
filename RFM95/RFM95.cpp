...
void RFM95::RFM_Send_Package(unsigned char *RFM_Tx_Package, unsigned char Package_Length, uint8_t SF)
{
  ...
  */
  ...
  // MOD:
  // Ensure RFM is in Tx mode:
  uint32_t start = millis();
  while(millis() - start < 1000){
    // Read RFM cfg register
    uint8_t currCfg  = RFM_Read(0x01);
    uint8_t LoRa_bit = (currCfg >> 7) & 0x01;
    uint8_t mode = (currCfg & 0b00000111);

    // Ensure module is in LoRa - Tx mode:
    if ((LoRa_bit == 1) and (mode == 3)) break;
  ...
  // MOD: add timeout to receive tx - done response
  delay(1);
